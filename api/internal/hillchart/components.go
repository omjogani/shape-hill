...
func drawBackground(svg *strings.Builder, height float64) {
...
func drawTitle(svg *strings.Builder, title string) {
	fmt.Fprintf(svg, `<text class="title" x="%.0f" y="34">%s</text>`, left, escape(title))
}

func drawSummit(svg *strings.Builder) {
	mid := x(50)
...
func drawScope(svg *strings.Builder, index int, dot Dot) {
	fill := dot.Color
	class := "ring"
	if dot.Stalled {
		fill = ""
...
func drawLegendEntry(svg *strings.Builder, index int, dot Dot) {
