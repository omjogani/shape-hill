...
import { usePublicScopeSnapshots } from "@/lib/hooks";
...
export function ScopeTimeline({
  ...
  const { data, isLoading, isError } = usePublicScopeSnapshots(scopeId, open);
  ...
  if (snapshots.length === 0) {
    return <p className="font-mono text-xs text-sage">No snapshots yet.</p>;
  }
